...
sudo gnome-terminal --tab -- sh -i -c "sudo ./out/mpc_server.out src/test/input/test.json; echo \"server ended\"; $END" && echo "server launched" && SERVER_PID=$(pidof ./out/mpc_server.out src/test/input/test.json) && echo "with PID: $SERVER_PID"
...
sudo gnome-terminal --tab -- sh -i -c "sudo ./out/mpc_ctrl.out src/test/input/test.json; echo \"ctrl ended\"; $END" && echo "ctrl launched" && CTRL_PID=$(pidof ./out/mpc_ctrl.out src/test/input/test.json) && echo "with PID: $CTRL_PID"
...
sudo gnome-terminal --tab -- sh -i -c "echo \"starting tracing\" && sudo trace-cmd record -e sched_switch -P $SERVER_PID,$CTRL_PID && echo \"tracing ended\" && echo \"making reports\" && trace-cmd report > doc/report/trace_sim.txt && echo \"extracting server trace\" && cat doc/report/trace_sim.txt | grep SERVER > doc/report/trace_server.txt && echo \"extracting ctrl trace\" && cat doc/report/trace_sim.txt | grep CLIENT > doc/report/trace_ctrl.txt && echo \"extrapolating server trace\" && python3 doc/report/filtro4.py doc/report/trace_server.txt print doc/report/trace_server_sum.csv && echo \"extrapolating ctrl trace\" && python3 doc/report/filtro4.py doc/report/trace_ctrl.txt print doc/report/trace_ctrl_sum.csv; sudo rm -v -f *.dat* *.out ; $END"
